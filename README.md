...
- **bool has_addr_arg** - является ли аргумент инструкции адресом.
...
- Удаление инструкции **pop**, следующей за **push** с тем же регистровым операндом;
- Уменьшение длины инструкции, если аргумент достаточно мал.

## ELF-файл

Следующим шагом массив инструкций транслируется в ELF-файл. Первым делом создаются заголовок ELF-файла и заголовок сегмента (как я это делал, можно посмотреть [здесь](/Manuals/elf.md)). В моём исполняемом файле всего один сегмент. Сегмент предоставляет доступ на чтение, запись и исполнение.
